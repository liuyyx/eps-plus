...
> Minecraft **26.3** 多加载器辅助客户端（Fabric / NeoForge）。
> 本仓库是 [NekoyaHouse/Epsilon](https://github.com/NekoyaHouse/Epsilon) 的分支，在保持上游架构不变的前提下，额外移植了几个模块。

<p align="left">
  <img alt="MC" src="https://img.shields.io/badge/Minecraft-26.3-4c1?style=flat-square">
...
| 加载器 | 文件 |
|---|---|
| Fabric | `epsilon-fabric-26.3-*.jar` |
| NeoForge | `epsilon-neoforge-26.3-*.jar` |
...
### Telly（移动 · 自动搭路）
自动 telly 搭桥 —— 边缘判据、方块候选搜索与放置策略移植自 RavenBS-Plus-Plus 的 telly 脚本，旋转与移动按真人可达速率重写（逐刻限速转角 + 沿用玩家自己的移动输入）。启用后按提示完成"武装"手势即可接管：

1. 站到方块边缘，**低头**（≥75°）并**朝向斜角** —— 会按 `Snap Degrees` 吸附到网格（默认 46.5°+k·90°）
2. **按住潜行** —— 屏上会出现一个高亮框，显示需要对准的方块面区域；满 `Activation Time` 后提示由红转绿
3. 提示变绿后，**按住右键并松开潜行** → 开始自动搭桥

屏幕提示会写明当前手势还差哪一步。相关设置：

- **Snap Degrees**（42~48，默认 **46.5**）—— 激活时吸附到的基准角网格，决定桥的走向。**默认值是实测选定的**：46.5 与 44 分处象限判定（`rawX = sin − cos` 的零点 45°）两侧，桥的走向轴不同 —— 实测 46.5 不掉、44 会掉。45° 本身既是整度数（吸附后会得到 `-315.00` 这类真人做不到的精确值，实测被 Intave 报 `acting computer-like`），又正好落在分界上（浮点抖动会让走向轴两侧跳），不建议使用。
- **Activation Time**（0.1~2.0 秒，默认 1.0）—— 提示由红转绿的等待时间。
- **Debug Log** —— 打开后按住潜行时每秒输出一次探测结果（九项判据逐条 ✓/✗ + 触发阶段），用于排查为什么无法触发。
- **Show Activation Hitbox** —— 是否绘制高亮框。
- **Auto Swap / Disable SafeWalk** —— 原脚本的配套行为。

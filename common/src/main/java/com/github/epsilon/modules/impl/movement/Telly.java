...
public class Telly extends Module {
    ...
    private boolean onPacketSent(Packet<?> packet) {
        ...
        if (packet instanceof ServerboundUseItemOnPacket placement) {
            BlockHitResult hit = placement.hitResult();
    ...
    private void restorePhysicalKey(String key) {
        ...
        setPressed(key, code >= 0 && InputConstants.isKeyDown(code));
    }
    ...
    private boolean autoPlaceOnPacketSent(Packet<?> packet) {
        ...
        if (packet instanceof ServerboundUseItemOnPacket c08) {
            BlockHitResult hit = c08.hitResult();
     ...
     * 触发条件里的「按住右键」永远不会成立。SDL 的物理按键状态不受此影响。</p>
     */
    private boolean isPhysicalMouseDown(int sdlButton) {
        return KeybindUtils.isPressed(KeybindUtils.encodeMouseButton(sdlButton));
    }

    private boolean physicalRightMouseDown() {
        return isPhysicalMouseDown(InputConstants.MOUSE_BUTTON_RIGHT);
    }

    private boolean physicalLeftMouseDown() {
        return isPhysicalMouseDown(InputConstants.MOUSE_BUTTON_LEFT);
    }
    ...
    private boolean isDropKeyName(int keyCode) {
        try {
            return InputConstants.Type.KEYBOARD.getOrCreate(keyCode).getName().toLowerCase(Locale.ROOT).contains("drop");
        } catch (Exception ignored) {
            return false;
        }
    }
